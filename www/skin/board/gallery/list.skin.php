			...
			<?php 
				...
				for ($i=0; $i < $list_cnt; $i++) { 
					...
					if ($list[$i]['icon_secret'] || $list[$i]['is_lock']) {
						...
						$wr_label = '<div class="label-band bg-black">Lock</div>';
						$is_lock = true;
					} else if ($list[$i]['icon_hot']) {
						$wr_icon = '<span class="wr-icon wr-hot"></span>';
						$wr_label = '<div class="label-band bg-red">Hot</div>';
					} else if ($list[$i]['icon_new']) {
						$wr_icon = '<span class="wr-icon wr-new"></span>';
						$wr_label = '<div class="label-band bg-blue">New</div>';
					}

					if($wr_id && $list[$i]['wr_id'] == $wr_id) {
						$wr_label = '<div class="label-band bg-green">Now</div>';
					}
