...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    if ($comment_id) {
		$response_flag = 'reply';
		$sql = " select wr_id, wr_comment, wr_comment_reply, wr_name from $write_table
	...
	// APMS : 내글반응 등록
	if($is_response) {
		apms_response('wr', 'comment', '', $bo_table, $wr_id, $wr_subject, $wr['mb_id'], $member['mb_id'], $member['mb_nick']);
		...
		if($response_flag == 'reply') { //대댓글일 때
			...
			apms_response('wr', 'comment_reply', '', $bo_table, $wr_id, $wr_subject, $pre_comment['mb_id'], $member['mb_id'], $member['mb_nick']);
		}
	}
