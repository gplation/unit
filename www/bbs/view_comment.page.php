...
// 댓글갯수
$crows = ($crows > 0) ? $crows : $board['as_comment_'.MOBILE_.'rows'];
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
	...
	if($is_content) {
		...
		$list[$i]['content'] = apms_content($list[$i]['content']).$lucky_msg;
	}
...
if($is_view_comment) {
	if ($is_comment_write)  {
		if(file_exists($board_skin_path.'/view_comment.page.script.php')) {
			include_once($board_skin_path.'/view_comment.page.script.php');
		} else {
