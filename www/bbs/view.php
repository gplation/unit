...
if($view['as_shingo'] < 0) {
	if($is_admin || ($view['mb_id'] && $view['mb_id'] == $member['mb_id'])) {
		$view['wr_content'] = '<p><b>블라인더 처리된 글입니다.</b></p>'.$view['wr_content'];
	} else {
		$view['wr_content'] = '<p><b>블라인더 처리된 글입니다.</b></p>';
	}
}
...
$view['content'] = $autoplay.apms_link_video($view['link']).$view['content'];
...
// SEO 메타태그
$seo_title = htmlspecialchars(apms_get_text($view['wr_subject']).' < '.$group['gr_subject'].' < '.$config['cf_title']);
$seo_subject = htmlspecialchars(apms_get_text($view['wr_subject']));
$seo_desc = htmlspecialchars(apms_cut_text($view['wr_content'], 200));
$seo_publisher = htmlspecialchars(apms_get_text($config['cf_title']));
$seo_creator = htmlspecialchars(apms_get_text($view['wr_name']));
$seo_keyword = '';
if($view['as_tag'] || $view['ca_name']) {
	$seo_keyword = ($view['as_tag']) ? $view['as_tag'] : $view['ca_name'];
	$seo_keyword = htmlspecialchars(apms_get_text($seo_keyword));
}
$seo_url = G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id;
$seo_img = apms_wr_thumbnail($bo_table, $view, 0, 0); // 썸네일
