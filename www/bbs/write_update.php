...
if ($w == '' || $w == 'r') {
    ...
    // 새글 INSERT
    if($is_new) sql_query(" insert into {$g5['board_new_table']} ( bo_table, wr_id, wr_parent, bn_datetime, mb_id, as_type, as_secret, as_publish, as_re_mb ) values ( '{$bo_table}', '{$wr_id}', '{$wr_id}', '".G5_TIME_YMDHIS."', '{$member['mb_id']}', '{$as_type}', '{$as_secret}', '{$publish}', '{$as_re_mb}' ) ");
    ...
    if ($w == '') {
		...
		if($is_response) {
			// APMS : 내글반응 등록
			apms_response('wr', 'reply', '', $bo_table, $wr_id, $wr_subject, $wr['mb_id'], $member['mb_id'], $member['mb_nick']);
		}
    ...
    */
    ...
    // 새글 UPDATE
    if($is_new) sql_query(" update {$g5['board_new_table']} set as_type = '{$as_type}', as_secret = '{$as_secret}', as_publish = '{$publish}' where bo_table = '{$bo_table}' and wr_id = '{$wr['wr_id']}' ", false);
}
