...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
    ...
    if ($board['bo_use_sideview']) {
		...
		$list['name'] = apms_sideview($list['mb_id'], $tmp_name, $list['wr_email'], $list['wr_homepage'], $list['as_level'], $lvl); // APMS 용으로 교체
	} else {
...
// OBJECT 태그의 XSS 막기
function apms_bad120422($matches)
{
    $tag  = $matches[1];
    $code = $matches[2];
...
// tag 내의 주석문 무효화 하기
function apms_bad130128($matches)
{
    $str = $matches[2];
...
// 내용을 G4 방식으로 필터링
function apms_conv_content($content) {
    global $config, $board;

	// XSS (Cross Site Script) 막기 - G4 코드복사해서 적용
	// 완벽한 XSS 방지는 없다.
	...
	$content = preg_replace_callback("#<(object|embed)([^>]+)>#i", "apms_bad120422", $content);
...
function conv_content($content, $html, $filter=true)
{
    ...
    if ($html)
    {
		...
		if($config['g4_conv_content']) { // G4 방식
			$content = preg_replace_callback("/<([^>]+)>/s", 'apms_bad130128', $content); 
	        $content = preg_replace($source, $target, $content);
		    $content = apms_conv_content($content);
		} else {
	        $content = preg_replace($source, $target, $content);
		    if($filter)
...
function chk_xp_num($point, $xp_point, $max_level, $xp_rate='0') {
	...
	if(!$point || $point < 0 || !is_numeric($point)) $point = 0;
...
function view_file_link($file, $width, $height, $content='')
{
    ...
    if (preg_match("/\.({$config['cf_image_extension']})$/i", $file)) {
		if($board['as_lightbox']) {
			apms_script('lightbox');
			$caption = ($content) ? ' data-title="'.$content.'"' : '';
	        $img = '<a href="'.G5_DATA_URL.'/file/'.$board['bo_table'].'/'.urlencode($file).'" target="_blank" data-lightbox="view-lightbox"'.$caption.'>';
...
function add_metatag($metatag, $order=0)
{
    ...
    if(trim($metatag))
        $html_process->merge_metatag($metatag, $order);
}
...
class html_process {
    ...
    function merge_metatag($metatag, $order)
    {
        $metas = $this->js;
        $is_merge = true;
    ...
    function run()
    {
		...
		<head>
		<meta charset="utf-8">
        밑으로 스킨의 메타가 위치하도록 하게 한다.
        */
        $buffer = preg_replace('#(<head>[^<]*<meta[^>]+>)#', "$1$metatag", $buffer);
