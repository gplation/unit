...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            $img_tag = $matches[0][$i];
            if(!$link && strpos($img_tag, G5_DATA_DIR.'/'.G5_EDITOR_DIR) && preg_match("/\.({$config['cf_image_extension']})$/i", $filename)) {
                $imgurl = str_replace(G5_URL, "", $src);
				if($board['as_lightbox']) {
					$caption = ($alt) ? ' data-title="'.$alt.'"' : '';
	                $thumb_tag = '<a href="'.$src.'" data-lightbox="view-ligghtbox"'.$caption.' target="_blank">'.$thumb_tag.'</a>';
				} else {
