...
function apms_event_info($bo_table, $wr_id, $row=null) {
	...
	// wr_1 : 시작일시
	// wr_2 : 종료일시
	// wr_3 : 당첨방법|당첨인원|참가비|참가비 증가율|입찰횟수|최소입찰|당첨자공개|최고가공개|당첨가공개
	// wr_4 : 총 입찰 횟수
	// wr_5 : 종료처리(1)
	// wr_6 : 협찬정보
	// wr_7 : 협찬회원 ID
	...
	if (!$row) {
		...
		$row = sql_fetch(" select wr_1, wr_2, wr_3, wr_4, wr_5, wr_6, wr_7, wr_8, wr_9, wr_10 from $tmp_write_table where wr_id = '$wr_id' ");
	}
			...
			for ($i=0; $row=sql_fetch_array($result); $i++) {
				...
				if(!$ev['type'] && $row['ev_point'] > 0) {
					...
					if($mb['mb_point'] >= $row['ev_point']) {
					    ...
					    insert_point($row['mb_id'], $ev_point, "{$board['bo_subject']} $wr_id 이벤트 당첨", $bo_table, $wr_id, "이벤트당첨");
					} else {
						$confirm = 0;
					}
				}
			    ...
			    // 실시간 쪽지 알림 기능
			    sql_query(" update {$g5['member_table']} set mb_memo_call = '{$config['cf_admin']}' where mb_id = '{$row['mb_id']}' ", false);
