...
if ($w == 'c') { // 댓글 입력
	...
	if ($comment_id) {
		$response_flag = 'reply';
		$sql = " select wr_id, mb_id, wr_comment, wr_comment_reply, wr_name from $comment_table where wr_id = '$comment_id' ";
	...
	apms_response('it', 'comment', $it_id, '', '', $it['it_name'], $it['pt_id'], $member['mb_id'], $member['mb_nick']);

	if($response_flag == 'reply' && $wr_re_mb && $wr_re_mb != $it['pt_id']) { //대댓글일 때 - 파트너와 다를 경우
		if($wr_re_mb != $member['mb_id']) { // 대댓글이 글쓴이와 다르다면 보냄
			apms_response('it', 'comment_reply', $it_id, '', '', $it['it_name'], $wr_re_mb, $member['mb_id'], $member['mb_nick']);
		}
	}
