...
if (!defined('_GNUBOARD_')) {
	...
	if(!defined('THEMA_PATH')) {
		if(!$ca_id) {
			$it = sql_fetch(" select it_id, ca_id, pt_id from {$g5['g5_shop_item_table']} where it_id = '$it_id' ");
			$ca_id = $it['ca_id'];
		}
...
$qrows = ($qrows > 0) ? $qrows : $itemrows['iqa_'.MOBILE_.'rows'];
...
$sql = "select * $sql_common order by iq_id desc limit $from_record, $qrows ";
...
$admin_href = ($is_admin == 'super' || IS_PARTNER) ? './myshop.php?mode=qalist' : '';
