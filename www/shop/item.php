...
/*
$event = array();
$result = sql_query(" select b.ev_id, b.ev_subject from `{$g5['g5_shop_event_item_table']}` a left join `{$g5['g5_shop_event_table']}` b on (a.ev_id = b.ev_id) where a.it_id = '{$it_id}' and b.ev_use = '1' order by b.ev_id ");
$i = 0;
while($row=sql_fetch_array($result)) { 
	$event[$i] = $row;
	$event[$i]['ev_mimg'] = G5_DATA_URL.'/event/'.$row['ev_id'].'_m';
	$event[$i]['ev_simg'] = G5_DATA_URL.'/event/'.$row['ev_id'].'_s';
	$event[$i]['ev_href'] = './event.php?ev_id='.$row['ev_id'];
	$i++;
}
$is_event = ($i > 0) ? true : false;
*/
...
$seo_desc = ($it['it_basic']) ? htmlspecialchars(apms_get_text($it['it_basic'])) : htmlspecialchars(apms_cut_text($it['it_explan'], 200));
$seo_publisher = htmlspecialchars(apms_get_text($config['cf_title']));
$seo_creator = ($author['mb_nick']) ? htmlspecialchars($author['mb_nick']) : $seo_publisher;
$seo_keyword = ($it['pt_tag']) ? htmlspecialchars($it['pt_tag']) : '';
$seo_url = G5_SHOP_URL.'/item.php?it_id='.$it['it_id'];
$seo_img = apms_it_thumbnail($it, 0, 0, false, true);

$metatag = PHP_EOL.'<meta name="title" content="'.$seo_title.'" />';
$metatag .= PHP_EOL.'<meta name="subject" content="'.$seo_subject.'" />';
$metatag .= PHP_EOL.'<meta name="publisher" content="'.$seo_publisher.'" />';
$metatag .= PHP_EOL.'<meta name="author" content="'.$seo_creator.'" />';
$metatag .= PHP_EOL.'<meta name="robots" content="index,follow" />';
if($seo_keyword) {
	$metatag .= PHP_EOL.'<meta name="keywords" content="'.$seo_keyword.'" />';
}
if($seo_desc) {
	$metatag .= PHP_EOL.'<meta name="description" content="'.$seo_desc.'" />';
}
$metatag .= PHP_EOL.'<meta property="og:title" content="'.$seo_title.'"/>';
$metatag .= PHP_EOL.'<meta property="og:site_name" content="'.$seo_publisher.'"/>';
$metatag .= PHP_EOL.'<meta property="og:author" content="'.$seo_creator.'" />';
$metatag .= PHP_EOL.'<meta property="og:url" content="'.$seo_url.'"/>';
if($seo_desc) {
	$metatag .= PHP_EOL.'<meta property="og:description" content="'.$seo_desc.'"/>';
}
if($seo_img['src']) {
	$metatag .= PHP_EOL.'<meta property="og:image" content="'.$seo_img['src'].'"/>';
}

add_metatag($metatag, 0);
...
if ($is_demo || $is_admin == 'super') {
    $setup_href = './skin.setup.php?skin=item&amp;ca_id='.urlencode($ca_id);
}
