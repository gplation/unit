...
if(IS_YC) {
	...
	// Shingo, Lucky, Code, Auto
	sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}` ADD `pt_shingo` tinyint(4) NOT NULL DEFAULT '0' AFTER `as_mobile_color` ", false);
	...
	sql_query(" ALTER TABLE `{$g5['g5_shop_cart_table']}`
					...
					ADD `pt_it` tinyint(4) NOT NULL DEFAULT '0',
	...
	sql_query(" ALTER TABLE `{$g5['g5_shop_event_table']}` ADD `ev_set` text NOT NULL DEFAULT '' ", false);
	sql_query(" ALTER TABLE `{$g5['g5_shop_event_table']}` ADD `ev_mobile_set` text NOT NULL DEFAULT '' ", false);
...
// Page -Line
sql_query(" ALTER TABLE `{$g5['apms_page']}` ADD `as_line` varchar(255) NOT NULL DEFAULT '' AFTER `as_target` ", false);
...
sql_query(" ALTER TABLE `{$g5['board_table']}`
				...
				ADD `as_line` varchar(255) NOT NULL DEFAULT '',
				ADD `as_show` tinyint(4) NOT NULL DEFAULT '0',
				ADD `as_order` tinyint(4) NOT NULL DEFAULT '0',
				ADD `as_menu` tinyint(4) NOT NULL DEFAULT '0',
				ADD `as_menu_show` tinyint(4) NOT NULL DEFAULT '0',
				ADD `as_grade` tinyint(4) NOT NULL DEFAULT '0',
				ADD `as_equal` tinyint(4) NOT NULL DEFAULT '0',
...
sql_query(" ALTER TABLE `{$g5['board_table']}` ADD `as_save` tinyint(4) NOT NULL DEFAULT '0' AFTER `as_lucky` ", false);
sql_query(" ALTER TABLE `{$g5['board_table']}` ADD `as_code` tinyint(4) NOT NULL DEFAULT '0' AFTER `as_save` ", false);
sql_query(" ALTER TABLE `{$g5['board_table']}` ADD `as_lightbox` tinyint(4) NOT NULL DEFAULT '0' AFTER `as_code` ", false);
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	sql_query(" ALTER TABLE `{$g5['write_prefix']}{$row['bo_table']}` ADD `as_level` int(11) NOT NULL DEFAULT '1' AFTER `as_publish` ", false);
	...
	sql_query(" ALTER TABLE `{$g5['write_prefix']}{$row['bo_table']}` ADD `as_re_mb` varchar(20) NOT NULL DEFAULT '' AFTER `as_choice_mb` ", false);
...
sql_query(" ALTER TABLE `{$g5['board_new_table']}` ADD `as_secret` tinyint(4) NOT NULL DEFAULT '0' AFTER `as_type` ", false);
sql_query(" ALTER TABLE `{$g5['board_new_table']}` ADD `as_publish` tinyint(4) NOT NULL DEFAULT '0' AFTER `as_secret` ", false);
sql_query(" ALTER TABLE `{$g5['board_new_table']}` ADD `as_comment` int(11) NOT NULL DEFAULT '0' AFTER `as_publish` ", false);
...
sql_query(" ALTER TABLE `{$g5['member_table']}`
				...
				ADD `as_response` int(11) NOT NULL DEFAULT '0' ", false);

// Member - Membership Date
sql_query(" ALTER TABLE `{$g5['member_table']}` ADD `as_date` int(11) NOT NULL DEFAULT '0' AFTER `as_response` ", false);

// Make Dir
$dir_arr = array (
	G5_DATA_PATH.'/apms/',
	G5_DATA_PATH.'/apms/partner/',
	G5_DATA_PATH.'/apms/main/',
	G5_DATA_PATH.'/apms/page/',
	G5_DATA_PATH.'/apms/photo/',
	G5_DATA_PATH.'/apms/photo/temp',
	G5_DATA_PATH.'/apms/video/',
	G5_DATA_PATH.'/apms/background/',
	G5_DATA_PATH.'/apms/background/body/',
	G5_DATA_PATH.'/apms/background/title/',
	G5_DATA_PATH.'/apms/background/page/',
	G5_DATA_PATH.'/apms/background/banner/'
);
...
if(IS_YC) {
	...
	$PHtml[] = array('isearch', 'shop/search.php', '아이템검색', '{아이콘:search} Item Search', '아이템 검색');
	$PHtml[] = array('iuse', 'shop/itemuselist.php', '후기검색', '{아이콘:pencil} Item Review', '후기 검색');
	$PHtml[] = array('iqa', 'shop/itemqalist.php', '문의검색', '{아이콘:comments-o} Item Q & A', '문의 검색');
}
...
//array('html_id', 'as_file', 'bo_subject', 'as_title', 'as_desc');
$DHtml = array();
$DHtml[] = array('intro', 'introduction', '사이트 소개', '{아이콘:leaf} Introduction', '사이트 소개');
$DHtml[] = array('provision', 'provision', '이용약관', '{아이콘:check-circle} Provision', '사이트 이용약관');
$DHtml[] = array('privacy', 'privacy', '개인정보보호방침', '{아이콘:plus-circle} Privacy', '사이트 개인정보보호방침');
$DHtml[] = array('noemail', 'noemail', '이메일 무단수집 거부', '{아이콘:ban} Rejection of E-mail Collection', '이메일 무단수집 거부');
$DHtml[] = array('disclaimer', 'disclaimer', '책임의 한계와 법적고지', '{아이콘:minus-circle} Lines of Responsibility', '책임의 한계와 법적고지');
$DHtml[] = array('guide', 'guide', '이용안내', '{아이콘:info-circle} User Guide', '사이트 이용안내');
...
//파일복사
$file_arr = array();
$file_arr[] = array(G5_DATA_PATH.'/apms/page/', 'disclaimer.php');
$file_arr[] = array(G5_DATA_PATH.'/apms/page/', 'guide.php');
$file_arr[] = array(G5_DATA_PATH.'/apms/page/', 'introduction.php');
$file_arr[] = array(G5_DATA_PATH.'/apms/page/', 'noemail.php');
$file_arr[] = array(G5_DATA_PATH.'/apms/page/', 'privacy.php');
$file_arr[] = array(G5_DATA_PATH.'/apms/page/', 'provision.php');

for ($i=0; $i<count($file_arr); $i++) {
	$org_file = G5_ADMIN_PATH.'/apms_admin/sample/'.$file_arr[$i][1];
	$copy_file = $file_arr[$i][0].$file_arr[$i][1];
	if(!file_exists($copy_file)) {
		@chmod($org_file, G5_FILE_PERMISSION);
		@copy($org_file, $copy_file);
		@chmod($copy_file, G5_FILE_PERMISSION);
	}
}
